package Utilities.Reports.Extent
...
/**
 * Created by in02183 on 3/30/2016.
 */
class ExtentReportFactory extends BaseSpec{
    ...
    public static String rootDir = new File(".").getCanonicalPath() /*Absolute Path*/


    private synchronized static ExtentReports getExtentReport() {
        if (reporter == null) {
            // config file or global variables
            reporter = new ExtentReports(rootDir+System.getProperty("extent.reports.directory"), true, DisplayOrder.OLDEST_FIRST);
            File extentReportConfigFile = new File(rootDir+System.getProperty("extent.reports.config.file"));
